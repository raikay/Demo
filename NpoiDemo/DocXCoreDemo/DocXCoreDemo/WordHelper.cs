...
namespace DocXCoreDemo
{
    public class WordHelper
    {
        ...
        public static System.IO.MemoryStream Export(string tempFilePath, Dictionary<string, object> data)
        {
            System.IO.MemoryStream ms = new System.IO.MemoryStream();
            using (DocX document = DocX.Load(tempFilePath))
            {
                ReplaceDoc(document, data);
                ReplaceTable(document, data);
                ReplaceList(document, data);
                document.SaveAs(ms);
            }
            ms.Close();
            ms.Dispose();
